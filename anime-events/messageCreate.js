...
module.exports = async (bot, msg) => {
    ...
    if (cleanedContent.includes('nigga') || cleanedContent.includes('nigger'))
        ...
        await bot.api('DELETE', `/channels/${msg.channel_id}/messages/${msg.id}`);
        await bot.api('POST', `/channels/${msg.channel_id}/messages`, {
            embed: {
                title: "Uh oh!",
    ...
    if (Object.keys(autoMessageRoles).includes(msg.channel_id) && !bot.mainGuild.members.find(x => x.id === msg.author.id).roles.includes(autoMessageRoles[msg.channel_id]))
        return await bot.api('PUT', `/guilds/${msg.guild_id}/members/${msg.author.id}/roles/${autoMessageRoles[msg.channel_id]}`);
};